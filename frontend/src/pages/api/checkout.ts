...
export const POST: APIRoute = async ({ locals, request }) => {
  ...
  let body: { items?: { product_id: string; quantity: number }[]; customer?: { name: string; email: string; phone?: string; address?: string; identification?: { type: string; number: string } } };
  try {
  ...
  const preference: any = {
    ...
    payer: {
      ...
      identification: body.customer.identification || undefined,
    },
    back_urls: {
  ...
  const orderItems = body.items.map(item => {
    ...
    return `('${orderId}', '${p.id}', '${p.title.replace(/'/g, "''")}', ${item.quantity}, ${p.price}, ${Math.round(p.price * item.quantity * 100) / 100})`;
  });
